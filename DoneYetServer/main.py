...
_SEGNO_VENDOR_DIR = os.path.join(SCRIPTS_PYTHON_DIR, "_vendor", "segno_pkg")
if os.path.isdir(_SEGNO_VENDOR_DIR) and _SEGNO_VENDOR_DIR not in sys.path:
    sys.path.insert(0, _SEGNO_VENDOR_DIR)
...
def _build_share_qr_svg(data_value):
    payload = str(data_value or "").strip()
    if not payload:
        raise ValueError("Missing share data.")
    if segno is None:
        raise RuntimeError("QR generator unavailable.")
    qr = segno.make(payload, error="m")
    buffer = io.BytesIO()
    qr.save(buffer, kind="svg", scale=6, border=3, dark="#0b1220", light="#ffffff")
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/data":
            ...
            return
        if parsed.path == "/api/runtime_status" or parsed.path == "/api/service_status":
            self.handle_api_runtime_status()
            return
        if parsed.path == "/api/share_qr":
            self.handle_api_share_qr(parsed.query)
            return
    ...
    def handle_api_share_qr(self, raw_query):
        try:
            params = parse_qs(raw_query or "", keep_blank_values=False)
            share_data = (params.get("data") or [""])[0]
            svg_bytes = _build_share_qr_svg(share_data)
            self.send_response(200)
            ...
            self.send_error(400, f"Share QR Error: {str(exc)}")
